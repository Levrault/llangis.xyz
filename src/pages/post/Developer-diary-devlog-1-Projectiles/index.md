...
After a long first entry, here my second one. Don't worry, it should be lighter. If I take a look back at last week devlog, my goal was to create an object pool to manage all my future type of projectile (throwing spear, bow and magic). This wasn't achieve since I simply don't need object pooling in Godot. 
...
As an ex unity develop, I through object pooling would be necessary and it would be the most efficient solution. BUT in me research, I found the following [tweet](https://twitter.com/reduzio/status/1073284242086551552) of [Juan Linietsky](https://twitter.com/reduzio). 
...
So I was wrong and I'm happy about it because I can now came with a much simpler solution.



### Projectile Scene

First step, I need a **main** projectile scene. It will serve has template for all my projectile.  

*For all unity developer up there , Godot use a scene inheritance system. To explain real quick, let's say that there are the equivalent of prefabs. Since a since can inherit from other scene and will all sync together. An inherited scene can also be customize to differ from the parent scene.*

![projectile_scene](./projectile_scene.png)

Here a what my main scene look like and It's composed of 

- A KinematicBody: Since a spear and an arrow are physic object that follow the hard law of gravity, a kinematic body was use. 

- An Animation Player: I wanted a small animation when the object need to disappear, so a cheap animation player with a *Fly* and *Death* state was enough.
...
My main scene created, time to make children. I created a Spear and Arrow scene, both inherit from Projectile. I wasted some times remembering how to manage inherited AnimationPlayer. Yeah, because by default, AnimationPlayer will always stay synced with the parent. That why need to make it unique. Strangly finding how to dot it was pretty long but I finally found the hidden button `Make Sub-Resources Unique` that make my inherited AnimationPlayer unique.
...
Since I use a final state machine, creating and adding a new behavior to my character is alway the same thing.

### First step - Setting up the future new state in Character.gd

* In my `Character.tscn`, I got node named `States` that contain all my state (seems pretty logical at this point). That where I add a new state (node2D) linked with the future state script (e.g. `atk-throw-spear.gd`).

  ![states](./states.png)

* My new state created, I add my new animation in my AnimationPlayer (e.g. `AtkThrowSpear` to keep the same name) 

* The final step, for this part, is to link those new path in my main character script. 
...
Time to code my new state script (eg. `atk-throw-spear.gd`).  Every state inherit from state script, it's some kind of abstract class.
...
Oh that's right, I talked about creating a kinematic projectile and a non kinematic one. It simply didn't need to create the non-kinematic one. I just needed to set gravity to 0 on the physic2D script of my kinematic projectile. Same thing at the end, and I avoid useless code was avoid.
...
An another week that was pretty productive. Godot's scene approach seems so much faster and easier to understand to me then prefabs. The work flow itself is simpler to implement. I know that prefabs from Unity where really great but scene are just more easier to understand and to use.
